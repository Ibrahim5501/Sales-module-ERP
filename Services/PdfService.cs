...
namespace example2.Services
{
    public class PdfService : IPdfService
    {
        ...
        public byte[] GenerateFacturePdf(Facture facture)
        {
            ...
            using var stream = new MemoryStream();
            var document = new PdfDocument();
            document.Info.Title = $"Facture {facture.NumeroFacture}";
            ...
            // Colors
            var primaryColor = XColor.FromArgb(15, 23, 42);      // Deep Navy
            var accentColor  = XColor.FromArgb(220, 38, 38);     // Red accent (invoice)
            var textColor    = XColor.FromArgb(51, 65, 85);
            var lightBg      = XColor.FromArgb(255, 241, 241);   // Light red tint
            ...
            // Right header title
            gfx.DrawString("FACTURE", fontTitle, new XSolidBrush(primaryColor), page.Width.Point - margin - 90, yPos);
            gfx.DrawString($"N°: {facture.NumeroFacture ?? $"FAC-{facture.Id_Facture}"}", fontHeader, new XSolidBrush(textColor), page.Width.Point - margin - 130, yPos + 22);
            ...
            // Left Box: Facture Info
            gfx.DrawRectangle(new XSolidBrush(lightBg), margin, yPos, boxWidth, 105);
            gfx.DrawRectangle(new XPen(XColor.FromArgb(252, 165, 165)), margin, yPos, boxWidth, 105);

            double boxY = yPos + 15;
            gfx.DrawString("INFORMATIONS FACTURE", fontHeader, new XSolidBrush(primaryColor), margin + 10, boxY);
            boxY += 16;
            gfx.DrawString($"Date facture:  {facture.DateFacture:dd/MM/yyyy}", fontBody, new XSolidBrush(textColor), margin + 10, boxY);
            boxY += 14;
            gfx.DrawString($"Date échéance: {facture.DateEcheance:dd/MM/yyyy}", fontBody, new XSolidBrush(textColor), margin + 10, boxY);
            boxY += 14;
            gfx.DrawString($"Réf. Devis: {facture.Devis?.NumeroDevis ?? "N/A"}", fontBody, new XSolidBrush(textColor), margin + 10, boxY);
            boxY += 14;
            string modePaiement = facture.Devis?.ModePaiement ?? "Virement Bancaire";
            gfx.DrawString($"Mode paiement: {modePaiement}", fontBody, new XSolidBrush(textColor), margin + 10, boxY);
            boxY += 14;
            gfx.DrawString($"Statut: {facture.Statut}", fontBold, new XSolidBrush(accentColor), margin + 10, boxY);

            // Right Box: Client Info
            double rightBoxX = margin + boxWidth + 20;
            gfx.DrawRectangle(new XSolidBrush(lightBg), rightBoxX, yPos, boxWidth, 105);
            gfx.DrawRectangle(new XPen(XColor.FromArgb(252, 165, 165)), rightBoxX, yPos, boxWidth, 105);

            boxY = yPos + 15;
            gfx.DrawString("CLIENT", fontHeader, new XSolidBrush(primaryColor), rightBoxX + 10, boxY);
            boxY += 16;
            string clientNom = facture.Partenaire != null
                ? $"{facture.Partenaire.Nom} ({facture.Partenaire.Entreprise})"
                : $"Partenaire #{facture.Id_Partenaire}";
            gfx.DrawString(Truncate(clientNom, 35), fontBold, new XSolidBrush(textColor), rightBoxX + 10, boxY);
            boxY += 14;
            gfx.DrawString($"Email: {facture.Partenaire?.Email ?? "N/A"}", fontBody, new XSolidBrush(textColor), rightBoxX + 10, boxY);
            boxY += 14;
            gfx.DrawString($"Tél:   {facture.Partenaire?.Telephone ?? "N/A"}", fontBody, new XSolidBrush(textColor), rightBoxX + 10, boxY);
            boxY += 14;
            string adresse = facture.Devis?.AdresseFacturation ?? facture.Partenaire?.Adresse ?? "N/A";
            gfx.DrawString($"Adresse: {Truncate(adresse, 30)}", fontBody, new XSolidBrush(textColor), rightBoxX + 10, boxY);
            ...
            // --- TABLE ROWS (from Devis lines) ---
            bool alt = false;
            var lignes = facture.Devis?.Lignes ?? new List<DevisLigne>();
            ...
            // --- TOTALS BOX ---
            double totalsWidth     = 250;
            double totalsX         = page.Width.Point - margin - totalsWidth;
            bool   hasGlobalRemise = facture.Devis != null && facture.Devis.RemiseGlobale > 0;
            double totalsBoxHeight = hasGlobalRemise ? 115 : 95;

            gfx.DrawRectangle(new XSolidBrush(lightBg), totalsX, yPos, totalsWidth, totalsBoxHeight);
            gfx.DrawRectangle(new XPen(XColor.FromArgb(252, 165, 165)), totalsX, yPos, totalsWidth, totalsBoxHeight);

            double tY = yPos + 16;

            if (hasGlobalRemise && facture.Devis != null)
            {
                decimal subtotalHTLines = lignes.Sum(l => l.MontantHT);
                gfx.DrawString("Sous-Total HT:", fontBody, new XSolidBrush(textColor), totalsX + 10, tY);
                gfx.DrawString($"{subtotalHTLines:N3} TND", fontBold, new XSolidBrush(textColor), totalsX + 145, tY);
                tY += 16;

                string remGlobaleLabel = facture.Devis.TypeRemiseGlobale == "MontantFixe"
                    ? $"Remise glob. ({facture.Devis.RemiseGlobale:N3} DT):"
                    : $"Remise glob. ({facture.Devis.RemiseGlobale:0.##}%):";
                decimal remiseAmount = subtotalHTLines - facture.MontantHT;

                gfx.DrawString(remGlobaleLabel, fontBody, new XSolidBrush(textColor), totalsX + 10, tY);
                gfx.DrawString($"-{remiseAmount:N3} TND", fontBold, new XSolidBrush(textColor), totalsX + 145, tY);
                tY += 16;
            }

            gfx.DrawString("Total HT:",  fontBody,   new XSolidBrush(textColor),    totalsX + 10,  tY);
            gfx.DrawString($"{facture.MontantHT:N3} TND",    fontBold, new XSolidBrush(textColor), totalsX + 145, tY);
            tY += 16;

            gfx.DrawString("Total TVA:", fontBody,   new XSolidBrush(textColor),    totalsX + 10,  tY);
            gfx.DrawString($"{facture.MontantTVA:N3} TND",   fontBold, new XSolidBrush(textColor), totalsX + 145, tY);
            tY += 16;

            gfx.DrawString("Montant Payé:", fontBody, new XSolidBrush(textColor),   totalsX + 10,  tY);
            gfx.DrawString($"{facture.MontantPaye:N3} TND",  fontBold, new XSolidBrush(textColor), totalsX + 145, tY);
            tY += 20;

            gfx.DrawLine(new XPen(accentColor, 1), totalsX + 10, tY - 12, totalsX + totalsWidth - 10, tY - 12);
            gfx.DrawString("Total TTC:", fontHeader, new XSolidBrush(primaryColor), totalsX + 10,  tY);
            gfx.DrawString($"{facture.MontantTotal:N3} TND",  new XFont("Arial", 10, XFontStyle.Bold), new XSolidBrush(accentColor), totalsX + 130, tY);
            tY += 18;

            gfx.DrawString("Reste à payer:", fontHeader, new XSolidBrush(primaryColor), totalsX + 10, tY);
            gfx.DrawString($"{facture.MontantRestant:N3} TND", new XFont("Arial", 10, XFontStyle.Bold), new XSolidBrush(accentColor), totalsX + 130, tY);

            // --- MONTANT EN MOTS ---
            double textWordsY = Math.Max(yPos + totalsBoxHeight + 15, yPos + 100);
            string words       = FrenchNumberToWordsConverter.ConvertToFrenchWords(facture.MontantTotal);
            string phraseWords = $"Arrêté la présente facture à la somme de : {words}.";

            gfx.DrawRectangle(new XSolidBrush(XColor.FromArgb(255, 248, 248)), margin, textWordsY, pageWidth, 28);
            gfx.DrawRectangle(new XPen(XColor.FromArgb(252, 165, 165)), margin, textWordsY, pageWidth, 28);
            gfx.DrawString(phraseWords, fontBold, new XSolidBrush(primaryColor), margin + 10, textWordsY + 18);
        ...
        public string SaveFacturePdf(Facture facture, string webRootPath)
        {
            var pdfBytes = GenerateFacturePdf(facture);
            var folder   = Path.Combine(webRootPath, "pdfs", "factures");
            if (!Directory.Exists(folder))
                Directory.CreateDirectory(folder);
            var fileName = $"Facture_{facture.NumeroFacture ?? facture.Id_Facture.ToString()}.pdf";
            var filePath = Path.Combine(folder, fileName);
            File.WriteAllBytes(filePath, pdfBytes);
            return $"/pdfs/factures/{fileName}";
        }
